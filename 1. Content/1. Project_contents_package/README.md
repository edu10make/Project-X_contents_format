...
## 1.4 class
프로젝트가 속해 있는 `분야`입니다. 
 - 학생들에게 프로젝트가 어떤 `분야`에 속해있으며, 프로젝트의 주제는 무엇인지 알려줍니다. 
 - 프로젝트의 적정 레벨을 `문자열`로 입력합니다. 
...
| Edu4 | "Front-end 분야의 중급 전문 지식을 이해하는 수준, 전체 개발 프로세스 내에서 FE영역의 개발을 경험해 본 수준 | 고가용성, 확장성, 유연성 등 안정적 서비스 지원을 위한 백엔드 아키텍쳐에 대한 설계가 가능 하고 구현을 할 수 있는 수준으로, FE, BE, Data, Infrastructure 에 대한 전체 구조 설계와 요건 정의 등이 가능함 |
