...
프로젝트를 구분하기 위해 각 프로젝트의 `id(고유 번호)`입니다. 
- 프로젝트의 고유번호는 4자리 `숫자`로 구성되며, 서로 다른 프로젝트는 동일한 `id`를 가질 수 없습니다.
...
프로젝트의 `제목`입니다. 
- 프로젝트 전체를 함축할 수 있는 문장이나 키워드를 입력합니다. 
...
프로젝트를 수행하는데 예상되는 소요시간입니다. 
- `숫자`로 입력하며 기본 단위는 '일'입니다. 
...
"estimated_time":50
```
- 해당 프로젝트를 수행하는데 예상되는 소요 시간은 50일입니다. 
...
따라서 프로젝트가 요구하는 `지식 수준`을 정의하기 위해서는 프로젝트가 지식을 요구하는 `분야`를 먼저 설정하고, 각 `분야`에서 요구하는 `지식 수준`을 정의해야 합니다. 
 - 학생들에게 프로젝트가 어떤 `분야`에 속해있으며, 프로젝트에서 요구하는 `지식 수준`은 어느정도인지 알려줍니다. 
 - 프로젝트의 `분야`와 `지식 수준`을 `분야/지식 수준`의 양식으로 `문자열`로 입력합니다. 
 - 입력하는 프로젝트의 `분야`와 `지식 수준`은 PSI에서 제공하는 [**'tech skill 역량명세서 - 수준 구분'**](https://drive.google.com/file/d/1vOcfcDQvWGrcwZy681pAUfqYj5tk9NED/view?usp=sharing)을 참고하여 작성합니다. 

### PSI tech skill 역량명세서 - 수준 구분
...
"class": [
 ...
 ```
 - 해당 프로젝트의 `분야`는 `AI-Data분석`, `AI-모델개발`, `AI-Ops`, `Back-end`이며 프로젝트의 수준은 Edu4입니다. 
