...
따라서 프로젝트가 요구하는 `지식 수준`을 정의하기 위해서는 프로젝트가 지식을 요구하는 `분야`를 먼저 설정하고, 각 `분야`에서 요구하는 `지식 수준`을 정의해야 합니다. 
 - 학생들에게 프로젝트가 어떤 `분야`에 속해있으며, 프로젝트의 `수준`은 어느정도인지 알려줍니다. 
 - 프로젝트의 분야와 `문자열`로 입력합니다. 
 - PSI에서 제공하는 [**'tech skill 역량명세서 - 수준 구분'**](https://drive.google.com/file/d/1vOcfcDQvWGrcwZy681pAUfqYj5tk9NED/view?usp=sharing) 의 양식에 맞춰 작성합니다. 
